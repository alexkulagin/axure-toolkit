...
/*
 ╔═════════════════════════════════════════════════════════════════╗
 ║       _                  ____            _       _              ║
 ║      | | __ ___   ____ _/ ___|  ___ _ __(_)_ __ | |_   • 3.1.6  ║
			...
			//┘	
				...
				const _applyExternals = function ()
				{
					!(function ()
					{
						...
						console.log('include GSAP custom plugin size 1.0.0');
						var _gsScope='undefined'!=typeof module&&module.exports&&'undefined'!=typeof global?global:this||window;(_gsScope._gsQueue||(_gsScope._gsQueue=[])).push(function(){'use strict';_gsScope._gsDefine.plugin({propName:'size',priority:0,API:2,version:'1.0.0',init:function(b,c,d){var f={origin:'0 0',width:null,height:null};for(var g in c)f[g]=c[g];var h=(f.origin||'0 0')+'',i=h.split(' '),j=i.length,k=f.width||Math.max(b.scrollWidth,b.clientWidth),m=f.height||Math.max(b.scrollHeight,b.clientHeight),n=d.vars.css,o=0<=n.width?n.width:k,p=0<=n.height?n.height:m,q=0,r=0;1===j&&-1!==i.indexOf('center')?q=r=0.5:(1===j||2===j)&&(q=-1===i.indexOf('left')?-1===i.indexOf('right')?i[0]:1:0,r=-1===i.indexOf('top')?-1===i.indexOf('bottom')?i[1]:1:0),q=1<Math.abs(q)?q/k:q,r=1<Math.abs(r)?r/m:r;var s=0<Math.abs(q)?k*q-o*q:0,t=0<Math.abs(r)?m*r-p*r:0;return n.x=0<=Math.abs(n.x)?n.x+s:s,n.y=0<=Math.abs(n.y)?n.y+t:t,!0}})}),_gsScope._gsDefine&&_gsScope._gsQueue.pop()();
				...
				//┘
					...
					PanelExtension.prototype = 
					{
						...
						setSizeTween: function (origin, durationIn, durationOut, easeIn, easeOut)
						{
							...
							var target = _.target.$().get()[0];

							var baseWidth = fit ? Math.max(target.scrollWidth, target.clientWidth) : Math.min(target.scrollWidth, target.clientWidth),
								baseHeight = fit ? Math.max(target.scrollHeight, target.clientHeight) : Math.min(target.scrollHeight, target.clientHeight);
							...
							var varsIN = {x:0,y:0};
								varsIN.width = baseWidth;
								varsIN.height = baseHeight;
								varsIN.ease = easeIn || undefined;
								varsIN.paused = true;
								varsIN.size = size;
							...
							var varsOUT = {x:0,y:0};
								varsOUT.width = 0;
								varsOUT.height = 0;
								varsOUT.ease = easeOut || undefined;
								varsOUT.paused = true;
								varsOUT.size = size;

							tween.in = TweenMax.to('#' + id, durationIn, varsIN);
							tween.out = TweenMax.to('#' + id, durationOut, varsOUT);

							tween.out.progress(1);
